...
async def create_user(session: AsyncSession, user:UserCreate) -> User:
    password = db_security.hash_password(user.password)
...
async def login_user(session:AsyncSession, username:str, password:str):
    ...
    if not user:
        return None
    if not db_security.verify_password(password, user.password):
        return None
    return user
...
async def create_refresh_token(data:dict):
    ...
    return jwt.encode(to_encode, settings.SECRET_KEY, algorithm=settings.ALGORITHM)# JWT中的编码方法jwt.encode()
async def verify_token(token:str,redis_client:redis.Redis=Depends(get_redis_client))-> TokenData|None:
    black_list = await redis_client.get(f"blacklist:{token}")
    if black_list:
        return None
    try:
